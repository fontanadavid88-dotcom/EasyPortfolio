...
export const MacroGauge: React.FC<MacroGaugeProps> = ({ value }) => {
  ...
  return (
    <div className="relative w-full h-48 flex flex-col items-center justify-center bg-panel rounded-2xl shadow-card border border-panel-border p-4">
      <h3 className="text-sm font-semibold text-panel-muted absolute top-4 left-4 flex items-center gap-1">
      ...
      <div className="absolute bottom-2 flex flex-col items-center">
        ...
        <span className="text-xs uppercase tracking-wider font-bold text-panel-muted">{currentZone}</span>
      </div>
    </div>
  );
};