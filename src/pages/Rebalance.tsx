...
export const Rebalance: React.FC = () => {
    ...
    const DeviationBlock = ({ deviation }: { deviation: number }) => {
        ...
        return (
            <div className="grid grid-cols-3 h-8 w-full gap-0.5 bg-backgroundDark rounded overflow-hidden border border-white/5">
                {/* Sottopesato (LEFT) */}
                <div className={`flex items-center justify-center relative ${isUnderweight ? 'bg-blue-900/40' : ''}`}>
                    {isUnderweight && (
                        <div
                ...
                {/* Neutro (CENTER) */}
                <div className={`flex items-center justify-center relative border-x border-white/5 ${isNeutral ? 'bg-yellow-900/20' : ''}`}>
                    {isNeutral && <div className="w-2 h-2 rounded-full bg-yellow-500 shadow-lg" />}
                </div>

                {/* Sovrapesato (RIGHT) */}
                <div className={`flex items-center justify-center relative ${isOverweight ? 'bg-red-900/40' : ''}`}>
                    {isOverweight && (
                        <div
    ...
    return (
        <div className="space-y-6 pb-20 animate-fade-in text-textPrimary">
            ...
            {/* HEADER CONTROLS */}
            <div className="bg-backgroundElevated p-6 rounded-2xl shadow-lg border border-borderSoft">
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    <div className="flex-1">
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-3">Strategia</label>
                        <div className="flex gap-3">
                            <button
                                ...
                                className={clsx(
                                    ...
                                    strategy === RebalanceStrategy.Accumulate ? "bg-primary border-primary text-white shadow-primary/30" : "border-borderSoft text-gray-400 hover:bg-white/5"
                                )}
                            >
                                Accumulo (Acquisti)
                            </button>
                            <button
                                ...
                                className={clsx(
                                    "flex-1 py-3 px-4 rounded-xl border text-sm font-bold transition-all shadow-lg",
                                    strategy === RebalanceStrategy.Maintain ? "bg-primary border-primary text-white shadow-primary/30" : "border-borderSoft text-gray-400 hover:bg-white/5"
                                )}
                            >
                                Mantenimento
                            </button>
                        ...
                        <div className="flex-1">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full border border-borderSoft bg-backgroundDark p-3 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none font-mono text-lg font-bold text-white text-right"
                                />
                            </div>
                        </div>
                    )}
            ...
            {/* HEATMAP TABLE */}
            <div className="bg-backgroundElevated rounded-2xl shadow-lg border border-borderSoft overflow-hidden overflow-x-auto">
                <table className="w-full text-sm text-left">
                    <thead className="bg-backgroundDark/50 text-gray-400 border-b border-borderSoft text-xs uppercase tracking-wider">
                        <tr>
                            ...
                            <th className="px-0 py-4 font-bold text-center w-[30%] bg-black/20">
                            ...
                            return (
                                <React.Fragment key={macro}>
                                    ...
                                    <tr className="bg-white/5 border-b border-white/5">
                                        ...
                                        <td className="px-2 py-3 font-bold text-center text-gray-300">{group.totalTargetPct.toFixed(1)}%</td>
                                        <td className="px-2 py-3 font-bold text-center text-gray-300">{group.totalCurrentPct.toFixed(1)}%</td>
                                        ...
                                        return (
                                            <tr key={p.ticker} className="hover:bg-white/5 transition-colors group">
                                                ...
                                                <td className="px-4 py-3 pl-8">
                                                    ...
                                                    <div className="text-xs text-gray-500 truncate max-w-[180px] group-hover:text-gray-400">{p.name}</div>
                                                </td>
                                                ...
                                                <td className="px-2 py-3 text-center">
                                                    {isEditing ? (
                                                        <input
                                                            autoFocus
                                                            className="w-14 border border-primary rounded bg-backgroundDark text-center font-bold text-white text-xs py-1 outline-none"
                                                            value={tempTargetVal}
                                                        ...
                                                        <div
                                                            ...
                                                            className="cursor-pointer hover:text-primary transition-colors font-mono font-medium text-gray-400 decoration-dotted underline underline-offset-4"
            ...
            <div className="text-center text-xs text-gray-500 mt-4">
                Valore Totale Stimato Post-Ribilanciamento: CHF <span className="font-bold text-gray-300">{(state.totalValue + (strategy === RebalanceStrategy.Accumulate ? cashInjection : 0)).toLocaleString()}</span>
            </div>
        </div>
    );
};