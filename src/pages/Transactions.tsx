...
export const Transactions: React.FC = () => {
    ...
    return (
        <div className="space-y-6 relative animate-fade-in text-textPrimary">
            ...
            <div className="flex justify-between items-center bg-backgroundElevated p-6 rounded-2xl shadow-lg border border-borderSoft sticky top-0 z-10 transition-colors">
                ...
                <button
                    ...
                    className="bg-primary text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-600 transition-all shadow-lg hover:shadow-primary/30 flex items-center gap-2"
                >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-backgroundElevated rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] overflow-y-auto border border-borderSoft">
                        <div className="p-6 border-b border-borderSoft flex justify-between items-center sticky top-0 bg-backgroundElevated z-10">
                            ...
                            <button onClick={() => setAssetModalOpen(false)} className="text-gray-400 hover:text-white transition-colors">
                        ...
                        <form onSubmit={handleSaveAsset} className="p-6 space-y-5">
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full border border-borderSoft bg-backgroundDark p-3 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none uppercase font-mono text-sm text-white"
                                    autoComplete="off"
                                />
                                {/* Search Preview Dropdown */}
                                {tickerSearchResults.length > 0 && (
                                    <div className="absolute top-full left-0 right-0 bg-backgroundElevated border border-borderSoft rounded-xl shadow-xl mt-1 z-20 max-h-48 overflow-y-auto">
                                        <div className="p-2 text-xs text-gray-500 font-medium bg-backgroundDark/50">Suggerimenti Borsa</div>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full border border-borderSoft bg-backgroundDark p-3 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none text-sm text-white"
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full border border-borderSoft bg-backgroundDark p-3 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none text-sm text-white dark-date-input"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full border border-borderSoft bg-backgroundDark p-3 rounded-xl focus:ring-2 focus:ring-primary focus:outline-none text-sm font-mono text-white"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="pt-4">
                                <button type="submit" className="w-full bg-primary text-white py-3.5 rounded-xl font-bold hover:bg-blue-600 transition shadow-lg text-sm">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-backgroundElevated rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-borderSoft">
                        <div className="p-6 border-b border-borderSoft flex justify-between items-center bg-white/5">
                            ...
                            <button onClick={() => setTxModalOpen(false)} className="text-gray-400 hover:text-white">
                                <span className="material-symbols-outlined">close</span>
                            </button>
                        </div>
            ...
            <div className="space-y-3">
                {groupedAssets.map(group => (
                    <div key={group.ticker} className="bg-backgroundElevated rounded-xl shadow-lg border border-borderSoft overflow-hidden transition-all hover:border-primary/30">

                        {/* Summary Header */}
                        <div
                            onClick={() => toggleGroup(group.ticker)}
                            className="p-4 flex items-center justify-between cursor-pointer hover:bg-white/5 select-none transition-colors"
                        >
                            <div className="flex items-center gap-4">
                                <div className={clsx(
                                    "p-2.5 rounded-full transition-colors",
                                    expandedTicker === group.ticker ? "bg-primary/20 text-primary" : "bg-white/5 text-gray-400"
                                )}>
                                    {group.ticker === 'CASH'
                                ...
                                <div>
                                    <h3 className="font-bold text-white text-sm">{group.ticker}</h3>
                                    <p className="text-xs text-gray-400">{group.quantity.toLocaleString()} quote • Avg: {group.avgPrice.toFixed(2)} {group.currency}</p>
                                </div>
                            </div>

                            <div className="flex items-center gap-6">
                                <div className="text-right hidden sm:block">
                                    <p className="text-sm font-bold text-white tracking-tight">{group.currentValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {group.currency}</p>
                                ...
                                <span className={`material-symbols-outlined text-gray-500 transition-transform duration-300 ${expandedTicker === group.ticker ? 'rotate-180' : ''}`}>
                                    expand_more
                                </span>
                            </div>
                        </div>
                        ...
                        {expandedTicker === group.ticker && (
                            <div className="bg-backgroundDark/50 border-t border-borderSoft animate-fade-in">
                                <table className="w-full text-sm text-left">
                                    <thead className="text-xs text-gray-500 uppercase bg-black/20 border-b border-borderSoft">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-borderSoft">
                                        {group.transactions.map(t => (
                                            <tr key={t.id} className="hover:bg-white/5 transition-colors group/row">
                                                <td className="px-6 py-3 font-medium text-gray-300">{new Date(t.date).toLocaleDateString('it-IT')}</td>
                                                <td className="px-6 py-3">
                                                    <span className={clsx(
                                                        "px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider",
                                                        t.type === TransactionType.Buy ? "bg-green-500/20 text-green-400" :
                                                            t.type === TransactionType.Sell ? "bg-red-500/20 text-red-400" :
                                                                "bg-blue-500/20 text-blue-400"
                                                    )}>
                                                        {t.type === TransactionType.Buy ? 'Acquisto' : t.type === TransactionType.Sell ? 'Vendita' : t.type}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-3 text-right text-gray-300 font-mono">{t.price.toFixed(2)}</td>
                                                <td className="px-6 py-3 text-right text-gray-300 font-mono">{t.quantity}</td>
                                                <td className="px-6 py-3 text-right text-gray-500 text-xs font-mono">{t.fees > 0 ? t.fees.toFixed(2) : '-'}</td>
                                                <td className="px-6 py-3 text-right font-bold text-gray-200 font-mono">{(t.quantity * t.price).toFixed(2)}</td>
                                                <td className="px-6 py-3 text-right">
                                                    <div className="flex items-center justify-center gap-1 opacity-0 group-hover/row:opacity-100 transition-opacity">
                                                        ...
                                                        <button
                                                            ...
                                                            className="p-1.5 rounded-lg hover:bg-white/10 text-gray-400 hover:text-negative transition-colors"
                                                            title="Elimina"
                                                        >
                                ...
                                <div className="p-3 text-center border-t border-borderSoft">
                                    <button
                                        ...
                                        className="text-xs text-primary font-bold uppercase tracking-wide hover:text-blue-400 flex items-center justify-center gap-1 w-full py-2 hover:bg-white/5 rounded transition-colors"
                                    >
                ...
                {!transactions?.length && (
                    <div className="text-center py-16 text-gray-500 bg-backgroundElevated rounded-2xl border border-dashed border-borderSoft">
